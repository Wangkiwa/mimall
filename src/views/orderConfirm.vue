  ...
  export default {
    ...
    data() {
      return {
        ...
        showDelModal: false, //是否显示删除modal
        checkedIndex: 0,
      }
    },
    components: {
    ...
    methods: {
      ...
      getCartList() {
        this.axios.get("/carts").then(res => {
          ...
          this.count = this.cartList.reduce(
      ...
      submitAddress(item) {
        ...
        if (userAction == 0 || userAction == 1) {
          ...
          if (!checkedItem.receiverName) {
            ...
            !checkedItem.receiverMobile ||
            !/\d{11}/.test(checkedItem.receiverMobile)
          ) {
            errMsg = "请输入正确的手机号"
            ...
            !checkedItem.receiverAddress ||
            !checkedItem.receiverDistrict
          ) {
            errMsg = "请输入收货地址"
          } else if (!/\d{6}/.test(checkedItem.receiverZip)) {
            errMsg = "请输入六位邮编"
          }
          if (errMsg) {
